...
namespace dsa::utils
{
   ...
   * or a specialization of std::hash for std::pair is required because the
   * standard library doesn't provide a default hash function for pairs. This is
   * needed because std::unordered_set relies on hashing to manage its elements
   * efficiently.
   * 
   * See dsa::algorithms::dp::coin_change_heap2 for an example usage.
   * 
  ...
  struct PairHash {
    ...
    size_t operator()(const std::pair<T1, T2>& p) const {
        // Combine hash values of the pair elements (^ is a bitwise XOR)
