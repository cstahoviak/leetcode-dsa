...
TEST(TestTwoPointers, TestTwoSum) {
  ...
  EXPECT_TRUE(two_pointers::two_sum(values, target_sum_valid));
  EXPECT_FALSE(two_pointers::two_sum(values, target_sum_invalid));

  // Solve the two-sum problem with a vector (dynamic array)
  std::vector<int> vec(values.begin(), values.end());
  EXPECT_TRUE(two_pointers::two_sum(vec, target_sum_valid));
  EXPECT_FALSE(two_pointers::two_sum(vec, target_sum_invalid));
}
...
TEST(TestTwoPointers, TestCombineSorted) {
  ...
  auto result_arr = two_pointers::combine_sorted(arr1, arr2);
  ...
  auto result2 = two_pointers::combine_sorted(vec1, vec2);
...
TEST(TestTwoPointers, TestIsSubsequence) {
  ...
  EXPECT_TRUE(two_pointers::is_subsequence(s, t));
}
...
TEST(TestTwoPointers, TestSortedSquares) {
  ...
  std::array<int, 6> result_arr = two_pointers::sorted_squares(arr);
  EXPECT_EQ(result_arr, expected_arr);

  // Solve using vectos (dynamic arrays)
  std::vector<int> vec(arr.begin(), arr.end());
  std::vector<int> expected_vec(expected_arr.begin(), expected_arr.end());
  std::vector<int> result_vec = two_pointers::sorted_squares(vec);
